...
mask_file = drive.lazyframe_from_file_name(service, file_name=mask_fn, folder_id=year_folder, drive_ft='csv', separator='|', infer_schma=False).collect()
prev_fn = f'AZ_{prev_year}{str(prev_month).zfill(2)}_masked.csv'
prev_file = drive.lazyframe_from_file_name(service, file_name=prev_fn, folder_id=prev_year_folder, drive_ft='csv', separator='|', infer_schma=False).collect()
